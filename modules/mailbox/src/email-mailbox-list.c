...
static void _mailbox_folders_name_cache_add_name(EmailMailboxView *view, int mailbox_id, const char *folder_name)
{
	...
	EmailFolderNameCashItem *cache_item = MEM_ALLOC(cache_item, 1);
...
static char *_mailbox_folders_name_cache_get_name(EmailMailboxView *view, int mailbox_id)
{
	...
	EmailFolderNameCashItem search_item = {
			.mailbox_id = mailbox_id,
			.mailbox_name = NULL
	};
	EmailFolderNameCashItem *search_result = eina_list_search_sorted(view->folders_names_cache, _mailbox_folder_name_cache_comparator_cb, &search_item);

	if (!search_result) {
		debug_log("Item not found in cache!");
		return NULL;
	} else {
		debug_log("Item found in cache! Mailbox name: %s", search_result->mailbox_name);
...
static int _mailbox_folder_name_cache_comparator_cb(const void *data1, const void *data2)
{
	const int mailbox_id1 = ((const EmailFolderNameCashItem *) data1)->mailbox_id;
	const int mailbox_id2 = ((const EmailFolderNameCashItem *) data2)->mailbox_id;
