...
static void _mailbox_create_date_range_popup(EmailMailboxView *view);
static void _mailbox_date_range_popup_cancel_btn_click_cb(void *data, Evas_Object *obj, void *event_info);
static void _mailbox_date_range_popup_done_btn_click_cb(void *data, Evas_Object *obj, void *event_info);
static void _mailbox_date_range_popup_del_cb(void *data, Evas *e, Evas_Object *obj, void *event_info);
static void _mailbox_date_range_popup_from_btn_click_cb(void *data, Evas_Object *obj, void *event_info);
static void _mailbox_date_range_popup_to_btn_click_cb(void *data, Evas_Object *obj, void *event_info);
static void _mailbox_date_range_popup_update_date_btn(EmailMailboxView *view,  ServerSearchDateRangeType range_type);

/*Date picker popup*/
static void _mailbox_create_date_picker_popup(EmailMailboxView *view, ServerSearchDateRangeType range_type);
static void _mailbox_date_picker_popup_set_btn_click_cb(void *data, Evas_Object *obj, void *event_info);
static void _mailbox_date_picker_popup_cancel_btn_click_cb(void *data, Evas_Object *obj, void *event_info);
static void _mailbox_date_picker_popup_del_cb(void *data, Evas *e, Evas_Object *obj, void *event_info);
...
static void _mailbox_searchbar_entry_changed_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	view->search_entry_changed_timer = ecore_timer_add(0.5, _mailbox_searchbar_editfield_changed_timer_cb, (void *)view);
...
static void _mailbox_setup_server_search_mode(EmailMailboxView *view)
{
	...
	if (difftime(view->search_to_date, view->search_from_date) <= 0.0) {
		_mailbox_init_sever_search_date_range(view);
	}

	_mailbox_create_date_range_popup(view);
	debug_leave();
}
...
{
	debug_enter();
	retm_if(difftime(view->search_to_date, new_from_date) < 0.0, "Attempt to set from_time greater than to_time! Rejected");

	view->search_from_date = new_from_date;
	if (view->date_btn_from) {
		_mailbox_date_range_popup_update_date_btn(view, SERVER_SEARCH_DATE_RANGE_FROM_DATE);
	}
...
{
	debug_enter();
	retm_if(difftime(new_to_date, view->search_from_date) < 0.0, "Attempt to set to_time less than from_time! Rejected");

	view->search_to_date = new_to_date;
	if (view->date_btn_to) {
		_mailbox_date_range_popup_update_date_btn(view, SERVER_SEARCH_SEARCH_DATE_RANGE_TO_DATE);
	}
...
static void _mailbox_init_sever_search_date_range(EmailMailboxView *view)
{
	...
	tm_time->tm_min = 59;
	tm_time->tm_sec = 59;
	view->search_to_date = mktime(tm_time);

	/*Setup default value of FROM Search time as one week later*/
	view->search_from_date = view->search_to_date - 7*24*60*60;
...
static void _mailbox_create_date_range_popup(EmailMailboxView *view)
{
	...
	Evas_Object *popup = mailbox_create_popup(view,
			...
			NULL,
			_mailbox_date_range_popup_cancel_btn_click_cb,
			_mailbox_date_range_popup_cancel_btn_click_cb,
			"IDS_EMAIL_BUTTON_CANCEL",
			_mailbox_date_range_popup_done_btn_click_cb,
			"IDS_EMAIL_BUTTON_DONE");
	evas_object_event_callback_add(popup, EVAS_CALLBACK_DEL, _mailbox_date_range_popup_del_cb, view);
	...
	evas_object_smart_callback_add(from_btn, "clicked", _mailbox_date_range_popup_from_btn_click_cb, view);
	...
	evas_object_smart_callback_add(to_btn, "clicked", _mailbox_date_range_popup_to_btn_click_cb, view);
...
static void _mailbox_date_range_popup_update_date_btn(EmailMailboxView *view, ServerSearchDateRangeType range_type)
{
	...
	char *formatted_date_text = NULL;

	if (range_type == SERVER_SEARCH_DATE_RANGE_FROM_DATE) {
		formatted_date_text = mailbox_get_formatted_date(view->search_from_date);
		btn = view->date_btn_from;
	} else if (range_type == SERVER_SEARCH_SEARCH_DATE_RANGE_TO_DATE) {
		formatted_date_text = mailbox_get_formatted_date(view->search_to_date);
		btn = view->date_btn_to;
	}

	elm_object_text_set(btn, formatted_date_text);
	FREE(formatted_date_text);
...
static void _mailbox_date_range_popup_from_btn_click_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	retm_if(!data, "Invalid arguments");
	EmailMailboxView *view = data;
	_mailbox_create_date_picker_popup(view, SERVER_SEARCH_DATE_RANGE_FROM_DATE);
...
static void _mailbox_date_range_popup_to_btn_click_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	retm_if(!data, "Invalid arguments");
	EmailMailboxView *view = data;
	_mailbox_create_date_picker_popup(view, SERVER_SEARCH_SEARCH_DATE_RANGE_TO_DATE);
	debug_leave();
}
...
static void _mailbox_create_date_picker_popup(EmailMailboxView *view, ServerSearchDateRangeType range_type)
{
	...
	Evas_Object *popup = mailbox_create_popup(view,
			...
			NULL,
			_mailbox_date_picker_popup_cancel_btn_click_cb,
			_mailbox_date_picker_popup_cancel_btn_click_cb,
			"IDS_EMAIL_BUTTON_CANCEL",
			_mailbox_date_picker_popup_set_btn_click_cb,
			"IDS_EMAIL_BUTTON_SET");

	evas_object_event_callback_add(datepicker_data->popup, EVAS_CALLBACK_DEL, _mailbox_date_picker_popup_del_cb, datepicker_data);

	Evas_Object *datetime = elm_datetime_add(popup);

	Elm_Datetime_Time from_time;
	memset(&from_time, 0, sizeof(Elm_Datetime_Time));
	localtime_r(&view->search_from_date, &from_time);

	Elm_Datetime_Time to_time;
	memset(&to_time, 0, sizeof(Elm_Datetime_Time));
	localtime_r(&view->search_to_date, &to_time);
...
static void _mailbox_date_picker_popup_set_btn_click_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	if (popup_data->range_type == SERVER_SEARCH_DATE_RANGE_FROM_DATE) {
		_mailbox_update_server_search_from_date_value(view, converted_time);
	} else if (popup_data->range_type == SERVER_SEARCH_SEARCH_DATE_RANGE_TO_DATE) {
		_mailbox_update_server_search_to_date_value(view, converted_time);
	}
