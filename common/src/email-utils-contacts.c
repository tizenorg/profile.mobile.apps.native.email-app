...
static Eina_List * _email_contacts_logs_db_search(const char *search_word, Eina_Hash *hash)
{
	...
	while (ret == CONTACTS_ERROR_NONE) {
		...
		if (!contact_info) {
			...
			email_contacts_delete_contact_info_list(&contacts_list);
			return NULL;
		}
...
{
	debug_enter();
	retm_if((!contact_info || !*contact_info), "Invalid parameter: contact_info[NULL]");
...
EMAIL_API void email_contacts_delete_contact_info_list(Eina_List **list)
{
	...
	retm_if(!list || !*list, "Invalid parameter: list is NULL!");
