...
static void _insert_tag_sub(char *dest, char *src, const char *key)
{
	...
	/* tagged key */
	strcat(dest, "<match>");
	strncat(dest, src + pre_len, STR_LEN((char *)key));
	strcat(dest, "</>");

	/* remainder */
	char *remainder = src + pre_len + STR_LEN((char *)key);
...
EMAIL_API GList *email_engine_get_mail_list_data(email_mail_list_item_t *mailbox_data, int mail_count, const char *folder_name, const char *key)
{
	...
	for (i = 0; i < mail_count; ++i) {
		...
		mailbox_info->account_id = mailbox_data[i].account_id;

		/* debug_log("mailbox_data[i].account_id : %d", mailbox_data[i].account_id); */

		if (STR_VALID(key)) {
			_insert_tag_sub(mailbox_info->subject, mailbox_data[i].subject, key);
			debug_secure("tagged_subject: [%s], key: [%s]", mailbox_info->subject, key);
			_insert_tag_sub(mailbox_info->from, mailbox_data[i].full_address_from, key);
		} else {
			STR_NCPY(mailbox_info->subject, mailbox_data[i].subject, SUBJECT_LEN);
			STR_NCPY(mailbox_info->from, mailbox_data[i].full_address_from, FROM_LEN);
		}
		STR_NCPY(mailbox_info->sender, mailbox_data[i].email_address_sender, ADDR_LEN);
		STR_NCPY(mailbox_info->recipient, mailbox_data[i].email_address_recipient, RECIPIENT_LEN);

		/* date time. */
		mailbox_info->datetime = mailbox_data[i].date_time;

		mailbox_info->seen = mailbox_data[i].flags_seen_field;

		/* priority. */
		mailbox_info->priority = mailbox_data[i].priority;

		/* folder_name */
		mailbox_info->mailbox_id = mailbox_data[i].mailbox_id;

		/* lock. */
		mailbox_info->lock = mailbox_data[i].lock_status;

		/* body download Y/N. */
		mailbox_info->body_download = mailbox_data[i].body_download_status;

		/* attachment Y/N. */
		mailbox_info->attachments = mailbox_data[i].attachment_count ? TRUE : FALSE;
		mailbox_info->thread_id = mailbox_data[i].thread_id;
		mailbox_info->thread_count = mailbox_data[i].thread_item_count;

		/* After prepending mails, reverse the list for efficiency. */
		mailbox_list = g_list_prepend(mailbox_list, mailbox_info);
	}
	mailbox_list = g_list_reverse(mailbox_list); /* list order is reserved */
	FREE(mailbox_data);
