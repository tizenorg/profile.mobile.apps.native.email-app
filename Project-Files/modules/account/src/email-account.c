...
static void _refresh_account_ug(EmailAccountView *view);
...
static int _account_module_create(email_module_t *self, email_params_h params)
{
	...
	email_params_get_int_opt(params, EMAIL_BUNDLE_KEY_IS_MAILBOX_MOVE_MODE, &is_move_mail_ug);
	...
	email_params_get_int_opt(params, EMAIL_BUNDLE_KEY_IS_MAILBOX_ACCOUNT_MODE, &is_account_list_ug);
	...
	debug_log("account_id [%d], folder_id [%d], is_move_mail_ug [%d], is_account_list_ug [%d]", view->account_id, view->folder_id, is_move_mail_ug, is_account_list_ug);
