...
typedef struct _EmailFilterAddVD EmailFilterVD;
...
static int _create(email_view_t *self)
{
	...
	navi_it = email_module_view_push(&vd->base, EMAIL_SETTING_STRING_ADD_PRIORITY_SENDER.id, 0);
	elm_object_item_domain_text_translatable_set(navi_it, EMAIL_SETTING_STRING_ADD_PRIORITY_SENDER.domain, EINA_TRUE);
...
static void _done_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	evas_object_smart_callback_del(obj, "clicked", _done_cb);
	...
	if (_checking_is_dup_rule(vd, filter_rule, ugd->op_type)) {
		...
		evas_object_smart_callback_add(obj, "clicked", _done_cb, vd);
		return;
	}
