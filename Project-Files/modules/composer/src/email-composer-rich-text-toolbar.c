...
static void _delete_popup(EmailComposerUGD *ugd);
...
static void _save_current_selection(EmailComposerUGD *ugd);
static void _restore_saved_selection(EmailComposerUGD *ugd);
static void _update_button_state(EmailComposerUGD *ugd, RichTextTypes type);
static void _init_toolbar_buttons_state(EmailComposerUGD *ugd);

static void _create_font_size_popup(EmailComposerUGD *ugd);

static void _update_all_buttons_state(EmailComposerUGD *ugd);
static void _update_property_button_state(EmailComposerUGD *ugd, RichTextTypes type);
static void _update_color_button_state(EmailComposerUGD *ugd, RichTextTypes type);
static void _create_option_selection_popup(EmailComposerUGD *ugd, RichTextTypes type);
static void _update_color_button_after_color_pick(EmailComposerUGD *ugd, RichTextTypes type);
static Evas_Object *_create_button(EmailComposerUGD *ugd, Evas_Object *parent, RichTextTypes type);

static Evas_Object *_create_color_image_layout(EmailComposerUGD *ugd, RichTextTypes type, const char *group_name);
...
static RichTextTypes _get_pressed_button_type(EmailComposerUGD *ugd, Evas_Object *button)
{
	...
	for (; i < RICH_TEXT_TYPE_COUNT; i++) {
		if (ugd->richtext_button_list[i].button == button) {
			return i;
		}
	}
...
static Evas_Object *_font_size_gl_content_get(void *data, Evas_Object *obj, const char *part)
{
	...
	FonSizesType *fontSize = (FonSizesType *)data;
	EmailComposerUGD *ugd = (EmailComposerUGD *)evas_object_data_get(obj, RICHTEXT_TOOLBAR_DATA_NAME);
	retvm_if(ugd == NULL, NULL, "rt_data is NULL!");
	Evas_Object *radio = NULL;

	if (!g_strcmp0(part, "elm.swallow.icon")) {
		radio = elm_radio_add(obj);
		elm_radio_group_add(radio, ugd->richtext_font_size_radio_group);
		elm_radio_state_value_set(radio, fontSize->index);

		if ((ugd->richtext_font_size_pixels >= fontSize->size_range_low) &&
				(ugd->richtext_font_size_pixels <= fontSize->size_range_high)) {
...
static void _font_size_gl_sel(void *data, Evas_Object *obj, void *event_info)
{
	...
	ugd->richtext_font_size_pixels = fontsize->size;

	_restore_saved_selection(ugd);
	...
	if (ewk_view_script_execute(ugd->ewk_view, script_string, NULL, NULL) == EINA_FALSE) {
...
static void _delete_popup(EmailComposerUGD *ugd)
{
	...
	retm_if(!ugd, "Invalid parameter: rt_data is NULL!");

	DELETE_EVAS_OBJECT(ugd->composer_popup);
	ugd->composer_popup = NULL;
	ugd->richtext_font_size_radio_group = NULL;
	ugd->richtext_colorselector = NULL;
...
static Evas_Object *_create_colorselector(Evas_Object *box, EmailComposerUGD *ugd, RichTextTypes type)
{
	debug_enter();
	...
	if (type == RICH_TEXT_TYPE_FONT_COLOR) {
		rgb = &ugd->richtext_font_color;
	} else if (type == RICH_TEXT_TYPE_BACKGROUND_COLOR) {
		rgb = &ugd->richtext_bg_color;
	}
...
static Evas_Object *_create_colorselector_popup_content(EmailComposerUGD *ugd, email_rgba_t rich_text_color, Evas_Object *parent, RichTextTypes type)
{
	...
	retvm_if(ugd == NULL, NULL, "ugd is NULL!");
	...
	ugd->richtext_colorselector = _create_colorselector(box, ugd, type);
	elm_box_pack_end(box, ugd->richtext_colorselector);
...
static void _update_color_button_after_color_pick(EmailComposerUGD *ugd, RichTextTypes type)
{
	...
	if (type == RICH_TEXT_TYPE_FONT_COLOR) {
		...
		color_cur = &ugd->richtext_bg_color;
	...
	elm_colorselector_color_get(ugd->richtext_colorselector,
	...
	if (email_util_get_rgb_string(szColor, sizeof(szColor),
		...
		_update_color_button_state(ugd, type);
	} else {
...
static void _font_color_response_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	_update_color_button_after_color_pick(ugd, RICH_TEXT_TYPE_FONT_COLOR);
...
static void _save_current_selection(EmailComposerUGD *ugd)
{
	if (ewk_view_script_execute(ugd->ewk_view, _update_cur_selection, NULL, NULL) == EINA_FALSE) {
		debug_error("ewk_view_script_execute failed!");
	}
}
...
static void _restore_saved_selection(EmailComposerUGD *ugd)
{
	if (ewk_view_script_execute(ugd->ewk_view, _restore_selection, NULL, NULL) == EINA_FALSE) {
		debug_error("ewk_view_script_execute failed!");
	}
}
...
static void _update_button_state(EmailComposerUGD *ugd, RichTextTypes type)
{
	...
	if (ewk_view_script_execute(ugd->ewk_view, command, NULL, NULL) == EINA_FALSE) {
		debug_error("ewk_view_script_execute failed!");
	}
}
...
static void _init_toolbar_buttons_state(EmailComposerUGD *ugd)
{
	if (ewk_view_script_execute(ugd->ewk_view,
...
static void _create_font_size_popup(EmailComposerUGD *ugd)
{
	...
	for (; index < sizeof(_rich_text_font_sizes) / sizeof(_rich_text_font_sizes[0]); index++) {
		...
		Elm_Object_Item *it = elm_genlist_item_append(genlist, &_font_size_itc, (void *)fontSize, NULL, ELM_GENLIST_ITEM_NONE, _font_size_gl_sel, ugd);

		bool found_font = (ugd->richtext_font_size_pixels >= fontSize->size_range_low) && (ugd->richtext_font_size_pixels <= fontSize->size_range_high);
		if (found_font) {
	...
	ugd->richtext_font_size_radio_group = radio;

	common_util_popup_display_genlist(ugd->composer_popup, genlist, EINA_FALSE, COMMON_POPUP_ITEM_1_LINE_HEIGHT, index);
...
static void _create_colorselector_popup(EmailComposerUGD *ugd, RichTextTypes type)
{
	...
	content = _create_colorselector_popup_content(ugd, *color_cur, ugd->composer_popup, type);
	elm_object_content_set(ugd->composer_popup, content);
...
static void _bg_color_response_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	_update_color_button_after_color_pick(ugd, RICH_TEXT_TYPE_BACKGROUND_COLOR);
...
static void _update_property_button_state(EmailComposerUGD *ugd, RichTextTypes type)
{
	Evas_Object *button = ugd->richtext_button_list[type].button;
	Eina_Bool state = ugd->richtext_button_list[type].state;
...
static void _update_color_button_state(EmailComposerUGD *ugd, RichTextTypes type)
{
	...
	if (type == RICH_TEXT_TYPE_FONT_COLOR) {
		...
		button = ugd->richtext_button_list[RICH_TEXT_TYPE_BACKGROUND_COLOR].button;
		color = &ugd->richtext_bg_color;
	} else {
		debug_error("wrong type [%d]", type);
		return;
	}
	...
	if((r != MAX_COLOR_VALUE || g != MAX_COLOR_VALUE || b != MAX_COLOR_VALUE) &&
		...
		select_color_img = _create_color_image_layout(ugd, type, EMAIL_IMAGE_COMPOSER_RTTB_BORDERED_COLOR_BAR);
	} else if ((r == MAX_COLOR_VALUE && g == MAX_COLOR_VALUE && b == MAX_COLOR_VALUE) &&
			(color->red != MAX_COLOR_VALUE || color->green != MAX_COLOR_VALUE || color->blue != MAX_COLOR_VALUE)) {
		evas_object_del(elm_object_part_content_unset(button, BUTTON_COLOR_LINE_PART_NAME));
		select_color_img = _create_color_image_layout(ugd, type, EMAIL_IMAGE_COMPOSER_RTTB_BORDERLESS_COLOR_BAR);
	}
...
static void _update_all_buttons_state(EmailComposerUGD *ugd)
{
	...
	for (; i < RICH_TEXT_TYPE_COUNT; i++) {
		if (i == RICH_TEXT_TYPE_FONT_COLOR || i == RICH_TEXT_TYPE_BACKGROUND_COLOR) {
			_update_color_button_state(ugd, i);
		} else {
			_update_property_button_state(ugd, i);
		}
	}
}
...
static void _create_option_selection_popup(EmailComposerUGD *ugd, RichTextTypes type)
{
	...
	_save_current_selection(ugd);

	switch (type) {
	case RICH_TEXT_TYPE_FONT_SIZE:
		_create_font_size_popup(ugd);
		break;
	case RICH_TEXT_TYPE_FONT_COLOR:
	case RICH_TEXT_TYPE_BACKGROUND_COLOR:
		_create_colorselector_popup(ugd, type);
		break;
	default:
...
static Evas_Object *_create_button(EmailComposerUGD *ugd, Evas_Object *parent, RichTextTypes type)
{
	...
	ugd->richtext_button_list[type].button = btn;
	...
	if (type == RICH_TEXT_TYPE_FONT_COLOR || type == RICH_TEXT_TYPE_BACKGROUND_COLOR) {
		...
		Evas_Object *select_color_img = _create_color_image_layout(ugd, type, group_name);
		evas_object_color_set(select_color_img, color->red, color->green, color->blue, color->alpha);
	}
...
static Evas_Object *_create_color_image_layout(EmailComposerUGD *ugd, RichTextTypes type, const char *group_name)
{
	Evas_Object *layout = elm_layout_add(ugd->richtext_button_list[type].button);
	...
	elm_object_part_content_set(ugd->richtext_button_list[type].button, BUTTON_COLOR_LINE_PART_NAME, layout);
...
EMAIL_API Evas_Object *composer_rich_text_create_toolbar(EmailComposerUGD *ugd)
{
	...
	Evas_Object *box = ugd->composer_box;
	...
	ugd->richtext_is_disable = EINA_FALSE;
	ugd->richtext_font_color = _default_font_colors[0];
	ugd->richtext_bg_color = _default_font_colors[1];

	_insert_box_padding_item(container_box, BOX_PADDING_OUTER);

	int i = RICH_TEXT_TYPE_FONT_SIZE;
	for (; i < RICH_TEXT_TYPE_COUNT; i++) {
		Evas_Object *btn = _create_button(ugd, container_box, i);
	...
	_init_toolbar_buttons_state(ugd);
...
EMAIL_API void composer_rich_text_disable_set(EmailComposerUGD *ugd, Eina_Bool is_disable)
{
	...
	debug_log("CURRENT STATE [%s] <> INCOMING STATE [%s]",
			(ugd->richtext_is_disable) ? "TRUE" : "FALSE",
			(is_disable) ? "TRUE" : "FALSE");

	if (ugd->richtext_is_disable == is_disable) {
		debug_leave();
		return;
	}
	ugd->richtext_is_disable = is_disable;
	...
	int i = RICH_TEXT_TYPE_FONT_SIZE;
	for (; i < RICH_TEXT_TYPE_COUNT; i++) {
		elm_object_disabled_set(ugd->richtext_button_list[i].button, is_disable);
		elm_object_signal_emit(ugd->richtext_button_list[i].button, button_signal, "");
	}

	if (!is_disable) {
		_update_all_buttons_state(ugd);
	}
...
EMAIL_API Eina_Bool composer_rich_text_disable_get(EmailComposerUGD *ugd)
{
	...
	debug_leave();
	return ugd->richtext_is_disable;
}
...
EMAIL_API void composer_rich_text_font_style_params_set(EmailComposerUGD *ugd, FontStyleParams *params)
{
	...
	ugd->richtext_font_size_pixels = params->font_size;
	...
	ugd->richtext_font_color.alpha = DEFAULT_ALFA_CHANNEL_VALUE;
	ugd->richtext_bg_color.alpha = DEFAULT_ALFA_CHANNEL_VALUE;

	if (!ugd->richtext_is_disable && evas_object_focus_get(ugd->ewk_view)) {
		_update_all_buttons_state(ugd);
	}
